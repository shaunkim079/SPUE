...
# id<-401013
# output_dir<-"output/gibbs_sampler_param_and_state_uncertainty_on_state_errors_real_data_split_periods_all_sites_state_values_seed_1"
...
loglikelihood_fun<-function(data,params){
  ...
  state_adjustments<-state_adjustments*params[1]
  ...
  Q1<-gr4j.run(param=params[1:4], initial_state_S=params[5]*params[1], initial_state_R=params[6]*params[3], 
               ...
               run_compiled = T,return_state=F,state_S_ts=state_values)/1000*data$area_m2/86400 #convert to cumecs
...
plot_function<-function(){
  ...
  burn.in<-10000 #4e5
  # if(i>burn.in){
  #   for(pp in 1:min(9,length(param.ranges))){
  #     plot(density(theta[burn.in:i,pp]))
  #     # plot(theta[burn.in:i,pp],type="l")
  #   }
  #   plot(density(theta[burn.in:i,29]))
  # } else {
  #   for(pp in 1:min(9,length(param.ranges))){
  #     plot(density(theta[1:i,pp]))
  #   }
  #   plot(density(theta[1:i,29]))
  # 
  # }
  if(i>burn.in){
    for(pp in 1:min(9,length(param.ranges))){
    ...
    plot(density(theta[burn.in:i,19]))
  } else {
    for(pp in 1:min(9,length(param.ranges))){
      plot(density(theta[1:i,pp]))
    }
    plot(density(theta[1:i,19]))
...
data<-list(cal.input=data.frame(P=input_ts$P,E=input_ts$E),
           area_m2=orig_params$area_m2,obs=input_ts$obs,num_of_adjustments=num_of_adjustments) #obs=input_ts$Q_no_state_errors #obs=input_ts$Q_with_state_errors
...
period_bounds<-floor(seq(0,nrow(data$cal.input)-1,length.out = num_of_adjustments+1))
start_periods<-period_bounds[-length(period_bounds)]+1
end_periods<-period_bounds[-1]

length_param_ranges_tmp<-length(param.ranges)
for(i in length_param_ranges_tmp:(length_param_ranges_tmp+num_of_adjustments-1)){
  param.ranges[[i+1]]<-c(start_periods[i-length_param_ranges_tmp+1],end_periods[i-length_param_ranges_tmp+1])
  # param.ranges[[i+1]]<-c(1,nrow(data$cal.input)-1)
}
for(i in length(param.ranges):(length(param.ranges)+num_of_adjustments-1)){
  param.ranges[[i+1]]<-c(0,3)
}
...
update_covariance_interval<-20 # this should be divide into tune_interval
