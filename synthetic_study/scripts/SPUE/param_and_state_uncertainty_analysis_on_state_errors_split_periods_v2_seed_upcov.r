...
if(length(theta_files)>0){
  ...
  # theta2<-NULL
  # for(i in length(theta_files):1){
  #   cat("reading theta",i,"/",length(theta_files),"\n")
  #   cur_theta2<-read.csv(theta_files[i],as.is=T)
  #   cur_theta2_new<-cur_theta2[500001:1e6,]
  #   theta2<-rbind(cur_theta2_new,theta2)
  # }
  
  restart_number<-5e5
  prev_seed<-as.numeric(gsub("theta_|.csv.gz","",basename(theta_files)))
  index_most_recent<-which(prev_seed==max(prev_seed))
  theta_file<-theta_files[index_most_recent]
  SD2_file<-paste0(output_dir,"/SD2_",prev_seed[index_most_recent],".csv.gz")
}
...
loglikelihood_fun<-function(data,params){
  ...
  if(is.na(Q1[1])){
    ...
    # acf(sigma_error)
    if(is.na(sigma_error[1]) | any(sigma_error<=0)){
      return(-Inf)
    } else {
      threshold<--100 #0.09 # TODO: adjust this based on rating curve
      
      indices_over_threshold<-which(obs_thinned>threshold)
...
data<-list(cal.input=data.frame(P=input_ts$P,E=input_ts$E),
           area_m2=orig_params$area_m2,obs=input_ts$Q_with_state_errors,num_of_adjustments=num_of_adjustments) #obs=input_ts$Q_no_state_errors #obs=input_ts$Q_with_state_errors
...
for(i in start_iter:ITER){
  ...
  if(tune_scale & i%%tune_interval==0){
    cat("log posterior prob:",L[i]+Pr[i],"\n")
